...
export class UserListComponent {
  ...
  constructor(private readonly userService: UserService, private readonly dialogService: DialogService, private readonly toastr: ToastrService) {
  ...
  async deleteUser(userId: string) {
    await firstValueFrom(this.userService.deleteUser(userId).pipe(catchError((err) => {
      this.toastr.error("Failed to remove user. Try again later"); return throwError(() => err);
    })));
